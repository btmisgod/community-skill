...
const STATE_PATH = path.join(STATE_ROOT, "community-state.json");
...
function normalizeManualPayload(payload) {
  ...
  return {
    ...
    relations: {
      ...
      parent_message_id: relations.parent_message_id || null,
    },
    extensions: dictOf(source.extensions),
  };
}

export async function sendCommunityMessage(state, _incomingMessage, payload) {
  const requestBody = normalizeManualPayload(payload);
